...
% joint_target = [0.1,0.4,0.3,0.0];
% dobot.PublishTargetJoint(joint_target);

%% Publish custom end effector pose
end_effector_position = [0.2,0,-0.04];
end_effector_rotation = [0,0,0.1];
dobot.PublishEndEffectorPose(end_effector_position,end_effector_rotation);
pause(2)

end_effector_position = [0.3,0.0001,0.01];
end_effector_rotation = [0,0,0];
dobot.PublishEndEffectorPose(end_effector_position,end_effector_rotation);

% 
% 
% end_effector_position = [0.2,0.11,1];
% end_effector_rotation = [0,0,0];
% dobot.PublishEndEffectorPose(end_effector_position,end_effector_rotation);% %% Turn on tool
% end_effector_position = [1,1,1];
% end_effector_rotation = [0,0,0];
% dobot.PublishEndEffectorPose(end_effector_position,end_effector_rotation);% %% Turn on tool
% % Open tool
% onOff = 1;
