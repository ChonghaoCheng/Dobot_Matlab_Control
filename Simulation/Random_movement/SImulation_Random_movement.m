...
for step =1:Total_step
    ...
    %Control method U1 : P control
    Control_value_P(step,:) = Cal_control_P(Target_line,X_obv,X_robo_P,step,X_obv_deviation);
    X_robo_P(end+1,:)=Robot_move(X_robo_P(end,:),Control_value_P(step,:));


    %Control method U2 : P control + compansation
    Control_value_P_C(step,:) = Cal_control_P_C(Target_line,X_obv,X_robo_P_C,step,X_obv_deviation);
    X_robo_P_C(end+1,:)=Robot_move(X_robo_P_C(end,:),Control_value_P_C(step,:));
    ...
    current_error = X_robo_current_PI-Transform_target(step,:);
    Control_error_sum = Control_error_sum+abs(current_error);
    Control_value_PI(step,:) = Cal_control_PI(Target_line,X_obv,X_robo_PI,step,X_obv_deviation,Control_error_sum);
    X_robo_PI(end+1,:) = Robot_move(X_robo_PI(end,:),Control_value_PI(step,:));
    % if Control_error_sum > 1.5
    %     Control_error_sum =0;
    % end
    ...
    plot(X_robo_plane_MPC(:,1),X_robo_plane_MPC(:,2),'g-o',LineWidth=1.5);
    plot(Target_line(:,1),Target_line(:,2),"k--x",LineWidth=1.5);
